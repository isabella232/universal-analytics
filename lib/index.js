...
Visitor.prototype = {
  ...
  pageview: function (path, hostname, title, params, fn) {
    ...
    params = Object.assign({}, this._context, this._persistentParams || {}, params);
  ...
  event: function (category, action, label, value, params, fn) {
    ...
    params.ec = category || params.ec;
    params.ea = action || params.ea;
    params.el = label || params.el;
    params.ev = value || params.ev;
    params.p = params.p || params.dp;
  ...
  ecAddImpression: function (listIndex, productIndex, impressionObject) {
    var params = Object.assign({}, this._context, this._persistentParams || {});

    utils.enhancedEcommerce.setProductImpressionListName(params, impressionObject.list, listIndex);
    utils.enhancedEcommerce.setProductImpressionSKU(params, impressionObject.id, listIndex, productIndex);
    utils.enhancedEcommerce.setProductImpressionName(params, impressionObject.name, listIndex, productIndex);
    utils.enhancedEcommerce.setProductImpressionBrand(params, impressionObject.brand, listIndex, productIndex);
    utils.enhancedEcommerce.setProductImpressionCategory(params, impressionObject.category, listIndex, productIndex);
    utils.enhancedEcommerce.setProductImpressionVariant(params, impressionObject.variant, listIndex, productIndex);
    utils.enhancedEcommerce.setProductImpressionPosition(params, impressionObject.position, listIndex, productIndex);
    utils.enhancedEcommerce.setProductImpressionPrice(params, impressionObject.price, listIndex, productIndex);

    this._tidyParameters(params);

    if ((!params["il" + listIndex + "pi" + productIndex + "id"] &&
        !params["il" + listIndex + "pi" + productIndex + "nm"])
    ) {
      return this._handleError("Please provide either a product sku or a product name");
    }

    return this._withContext(params);
  },


  ecAddProduct: function (productIndex, impressionObject) {
    var params = Object.assign({}, this._context, this._persistentParams || {});

    utils.enhancedEcommerce.setProductSKU(params, impressionObject.id, productIndex);
    utils.enhancedEcommerce.setProductName(params, impressionObject.name, productIndex);
    utils.enhancedEcommerce.setProductBrand(params, impressionObject.brand, productIndex);
    utils.enhancedEcommerce.setProductCategory(params, impressionObject.category, productIndex);
    utils.enhancedEcommerce.setProductVariant(params, impressionObject.variant, productIndex);
    utils.enhancedEcommerce.setProductPrice(params, impressionObject.price, productIndex);
    utils.enhancedEcommerce.setProductQuantity(params, impressionObject.quantity, productIndex);
    utils.enhancedEcommerce.setProductCouponCode(params, impressionObject.couponCode, productIndex);
    utils.enhancedEcommerce.setProductPosition(params, impressionObject.position, productIndex);

    this._tidyParameters(params);

    if (!params["pr" + productIndex + "id"] && !params["pr" + productIndex + "nm"]) {
      return this._handleError("Please provide either a product sku or a product name");
    }
    return this._withContext(params);
  },
  ...
  ecSetAction: function (action, actionObject) {
    if (typeof action === 'object') {
      actionObject = action;
      action = null;
    }

    var translatedAction = this._translateParams(actionObject);
    var params = Object.assign({}, this._context, this._persistentParams || {});

    params.pa = action || translatedAction.pa || params.pa || this._context.pa;
    params.ti = translatedAction.ti || params.ti || this._context.ti;
    params.ta = translatedAction.ta || params.ta || this._context.ta;
    params.tr = translatedAction.tr || params.tr || this._context.tr;
    params.tt = translatedAction.tt || params.tt || this._context.tt;
    params.ts = translatedAction.ts || params.ts || this._context.ts;
    params.tcc = translatedAction.tcc || params.tcc || this._context.tcc;
    params.pal = translatedAction.pal || params.pal || this._context.pal;
    params.cos = translatedAction.cos || params.cos || this._context.cos;
    params.col = translatedAction.col || params.col || this._context.col;
